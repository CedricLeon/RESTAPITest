...
class Adapter():
    ...
    def inputToEndpoint(self, inputs, debug):
        # --- DEBUG mode check ---
        if debug:
            print(f"{bcolors.OKBLUE}DEBUG MODE ACTIVATED{bcolors.ENDC}")
        ...
        if debug:
            print(f"Inputs: {inputs}, nb_inputs = {nb_inputs}, input1 = {input} and input2 = {input2}.")
        ...
        if debug:
            print(f"Length of the segment: {resource_lenght}, resource_lenght * nb_resources = {double_precision_check}")

        # --- Get resource name ---
        resource_index = math.ceil(nb_resources * input) - 1
        resource_name = APIContent[resource_index].lower()
        if debug:
            print(f"Resource index: \"{resource_index}\", Resource Name: \"{resource_name}\".")
        ...
        if debug:
            print(f"Endpoint lenght: \"{endpoint_lenght}\", Endpoint probability: \"{endpoint_prob}\".")

        endpoint_index = math.ceil(nb_endpoints * endpoint_prob * nb_resources) - 1
        endpoint_name = Endpoints[endpoint_index].lower()
        if debug:
            print(f"Endpoint index: \"{endpoint_index}\", Endpoint Name: \"{endpoint_name}\".")
        ...
        if debug:
            print(f"Calling: {switch_endpoints_dict[endpoint_name]}(self, {BASE}, {resource_name}, {input2}, {debug})")
    ...
    def getFakeRewardFromEndpoint(self, index):
        if index == 0:
            ...
            print(f"Fake Reward Computation: Index = {index}, Endpoint = DELETE, Return 100.")
            return 100
        else:
